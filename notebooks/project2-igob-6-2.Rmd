...
```{r plot_nn, fig.height=6, fig.width=9, message=FALSE, warning=FALSE, fig.cap="Graphical representation of the NN model with the weights on each connection:"}
plot(nn)
```
...
Let's try to explain clusters by the wine type. Code below builds a matrix whe columns are cluster numbers and rows are wine types. As we can see, we've got 6 well defined clusters clusters 3 and 4 contain different red wines, clusters 1, 2, 5 and 6 contain different types of white wines.

```{r}
table(wines_data[,13],k.means.fit$cluster)
```

Let's try to figure out how red wine clusters 3 and 4 differ from each other. Code below calculates a difference vector of cluster 1 and 2 and the sort the attributes in the order of the most influence:

```{r}
v12 <- k.means.fit$centers[3,] - k.means.fit$centers[4,]
v12 <- v12[order(abs(v12), decreasing = T)]
print(v12)
```

It looks that those red wines differ in acidity, specifically in content of citric asid (CA), but nut in sugar content and alcohol.Apparently the taste differently