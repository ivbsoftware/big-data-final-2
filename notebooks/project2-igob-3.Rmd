---
title: "Wine dataset notebook to find wine groups"
...
# Feature reduction
```{r}
wines_data_sm <- wines_data[c("RS","pH","DEN","ALC","QLT","TYPE")]
```
...
```{r}
wines_data_sm.std <- scale(wines_data_sm[1:4])
head(wines_data_sm.std)
```
...
wssplot(wines_data_sm.std, nc=10) 
```

## Red wine dataset clustering K-means
```{r  fig.height=10, fig.width=10}
clusters_num = 4
k.means.fit <- kmeans(wines_data_sm.std, clusters_num,iter.max = 1000)
...
```{r fig.height=10, fig.width=10}
library(cluster)
clusplot(wines_data_sm.std, k.means.fit$cluster, main='2D representation of the Cluster solution',
